...
public class Batch {
	public static void main(String[] args) throws JsonParseException, JsonMappingException, IOException {
		...
		JavaRDD<List<StockDetails>> sym = inputData.map(f -> new ObjectMapper().readValue(f, new TypeReference<List<StockDetails>>(){}));
		
		JavaRDD<StockDetails> stocks = sym.flatMap(new FlatMapFunction<List<StockDetails>, StockDetails>() {

			@Override
			public Iterator<StockDetails> call(List<StockDetails> arg0) throws Exception {
				List<StockDetails> sd = new ArrayList<>();
				arg0.forEach(s -> sd.add(s));
				return sd.iterator();
			}
		});
		
		JavaRDD<String> stockSymbols = stocks.map(f -> f.getSymbol()); 
		
		stockSymbols.saveAsTextFile("C:/Users/Thrishul/Desktop/test1.txt");
